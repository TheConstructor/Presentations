...
\begin{columns}
    \column{.60\textwidth}
        ...
        \item<2-> There is one setting to change it for the current session/tab (Query \textrightarrow{} Query Options…)\only<2>{\\\adjustimage{max width={1.0\columnwidth},max height={4cm},keepaspectratio}{graphics/menu_query_query_options}}
        \item<3-> and \textbf{another} to change it for new sessions (Tools \textrightarrow{} Options)\only<3>{\\\adjustimage{max width={0.9\columnwidth},max height={4cm},keepaspectratio}{graphics/menu_tools_options}}
        \end{itemize}
    \column{.40\textwidth}
        \begin{center}
