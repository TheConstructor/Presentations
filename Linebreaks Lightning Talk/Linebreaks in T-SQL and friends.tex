...
\begin{columns}
    \column{.60\textwidth}
        \begin{itemize}
            ...
            \item There is however 'Line Continuation' \only<1|handout:0>{\lstinputlisting[language=SQL,linerange=1-5]{demos/line-continuation.sql}}\only<2>{\lstinputlisting[language=SQL,linerange=1-6]{demos/line-continuation.sql}}
        \end{itemize}
    \column{.40\textwidth}
        \begin{center}
...
\begin{itemize}
    ...
    \item But you can do this \lstinputlisting[language=SQL,linerange=9-16]{demos/line-continuation.sql}
\end{itemize}
\end{frame}
...
\begin{columns}
    \column{.60\textwidth}
        ...
        \item<2-> There is one setting to change it for the current session/tab (\texttt{Query} \textrightarrow{} \texttt{Query Options…})\only<2|handout:0>{\\\adjustimage{max width={1.0\columnwidth},max height={4cm},keepaspectratio}{graphics/menu_query_query_options}}
        \item<3-> and \textbf{another} to change it for new sessions (\texttt{Tools} \textrightarrow{} \texttt{Options})\only<3|handout:0>{\\\adjustimage{max width={0.9\columnwidth},max height={4cm},keepaspectratio}{graphics/menu_tools_options}}
        \end{itemize}
    \column{.40\textwidth}
        \begin{center}
...
\begin{frame}{Git}
    ...
    \column{.40\textwidth}
        \begin{center}
            \adjustimage{max width={1.0\columnwidth},max height={7cm},keepaspectratio}{graphics/transport}%
        \end{center}
    \end{columns}
\end{frame}
